...
def zipfrombin(file, dev_type = 65535, dev_revision = 65535, application_version = 4294967295, sd_req = 65535):
  ...
  try:
    ...
    sys.exit('Could not open %s' % file)
  ...
  init_packet = []
  init_packet.extend([dev_type & 0xFF, (dev_type >> 8) & 0xFF])
  init_packet.extend([dev_revision & 0xFF, (dev_revision >> 8) & 0xFF])
  init_packet.extend([application_version & 0xFF, (application_version >> 8) & 0xFF, (application_version >> 16) & 0xFF, (application_version >> 24) & 0xFF])
  init_packet.extend([0x01, 0x00])
  init_packet.extend([sd_req & 0xFF, (sd_req >> 8) & 0xFF])
  init_packet.extend(convert_uint16_to_array(crc))

  manifest = _create_manifest(basename, dat_out_name, dev_type, dev_revision, application_version, sd_req, crc)
  ...
  archive.write(file, basename, zipfile.ZIP_DEFLATED)
  archive.close()
...
def main():
  ...
  parser.add_argument ('--sd-req', default=65535, type = int, help = 'The required softdevice (singular) as in integer, ex 100 for 0x64 for SD8.0')
  ...
  zipfrombin(args.file, args.dev_type, args.dev_revision, args.application_version, args.sd_req)
